...
export function ScheduleReport() {
  ...
  const fetchGeneratedFiles = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/schedule/files`);
  ...
  const handlePreview = (fileType, fileName) => {
    const fileUrl = `${import.meta.env.VITE_API_URL}/schedule/files/${fileType}/${fileName}/preview`;
  ...
  const handleDownload = (fileType, fileName) => {
    const fileUrl = `${import.meta.env.VITE_API_URL}/schedule/files/${fileType}/${fileName}/download`;
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `${import.meta.env.VITE_API_URL}/schedule/files/${fileType}/${fileName}`
          );
