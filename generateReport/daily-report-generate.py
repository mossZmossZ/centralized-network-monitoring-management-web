...
def header_footer(canvas, doc):
    ...
    - Left: "Centralized Monitoring Daily Report"
    - Center: "Confidential for internal use" (in red)
    - Right: "Report Date" from API
    """
    page_width, page_height = A4
    footer_y = 20  # Footer position
    ...
    # **Centered text: "Confidential for internal use" in RED**
    canvas.setFont("Helvetica-Bold", 10)
    canvas.setFillColor(colors.red)
    canvas.drawCentredString(page_width / 2, footer_y, "Confidential for internal use")
    canvas.setFillColor(colors.black)  # Reset fill color to black

    # Right-aligned: Report date (Dynamically from API)
